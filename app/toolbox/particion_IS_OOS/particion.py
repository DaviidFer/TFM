...
def split_is_oos_from_ready_df(
    ...
    if not (0.3 <= is_pct <= 0.7):
        raise ValueError(f"is_pct debe estar en [0.3, 0.7]. Valor recibido: {is_pct}")
    if not (0.3 <= oos_pct <= 0.7):
        raise ValueError(f"oos_pct debe estar en [0.3, 0.7]. Valor recibido: {oos_pct}")
