...
def diagnose_rule_returns(rule_returns: pd.DataFrame, name: str = "rule_returns", sample_cols: int = 200) -> None:
    ...
    for j in range(n_check):
        if np.array_equal(A[:, j], base):
            same += 1
    print(f"[{name}] columnas identicas a la col0 (en las primeras {n_check}): {same}")
...
def prune_correlated_rules_fast(
    ...
    fallback = {"mean": col_mean, "sum": A.sum(axis=0), "sharpe": col_mean / col_std}.get(metric, col_mean)
    if scores is None:
    ...
    ranking_df = pd.DataFrame(
        {"regla": cols, "score": scores, "ops": ops, "mean": means, "std": stds}
...
def run_pl_correlation_pruning(
    ...
    if df_rules_long is not None and not df_rules_long.empty:
        rr_long = build_rule_return_matrix(
            data=data_all, df_rules=df_rules_long, direction="long", return_col=return_col, chunk_size=chunk_size, dtype=dtype
        )
        if diagnose:
    ...
    if df_rules_short is not None and not df_rules_short.empty:
        rr_short = build_rule_return_matrix(
            data=data_all, df_rules=df_rules_short, direction="short", return_col=return_col, chunk_size=chunk_size, dtype=dtype
        )
        if diagnose:
