...
def combine_rule_dfs(rule_dfs: Iterable[pd.DataFrame] | Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for i, df in enumerate(dfs):
        ...
        if "regla" not in df.columns:
            raise ValueError(f"El DataFrame en posicion {i} no contiene la columna 'regla'.")
    ...
    if "regla" not in out.columns:
        raise ValueError("No se encontro columna 'regla' tras combinar rule_dfs.")
...
def monkey_validate_is(
    ...
    if not valid_mask_is.any():
        raise ValueError("No hay returns validos en IS.")
    ...
    print("=" * 90)
    print(f"VALIDACION IS | {direction.upper()}")
    print("=" * 90)
...
def monkey_validate_oos(
    ...
    if not valid_mask_oos.any():
        raise ValueError("No hay returns validos en OOS.")
    ...
    print("=" * 90)
    print(f"VALIDACION OOS | {direction.upper()}")
    print("=" * 90)
    print(f"Reglas entrada : {n_input}")
    print(f"Reglas aprobadas: {len(passed_df)}")
    print(f"Reglas eliminadas: {n_input - len(passed_df)}")
    print(f"Clausulas unicas cacheadas: {len(clause_cache)}")
...
__all__ = [
    ...
    "monkey_validate_is",
    "monkey_validate_oos",
    "monkey_validate_is_multi",
    "monkey_validate_oos_multi",
]
