...
def select_stable_rules(
    ...
    rules_to_eval = sorted(set(winners_rules).intersection(decor_rules)) if (decor_rules is not None and decorrelated_only) else sorted(set(winners_rules))
    ...
    score = w_sharpe * sharpe_pct + w_mono * monotonicity + w_mdd * mdd_score
    ...
    *,
    ...
    if rule_returns_long is None:
        ...
        rule_returns_long = build_rule_return_matrix(
            data=data, df_rules=rules_long_eval, direction="long", return_col=return_col, chunk_size=chunk_size, dtype=dtype
        )
    ...
    if rule_returns_short is None:
        ...
        rule_returns_short = build_rule_return_matrix(
            data=data, df_rules=rules_short_eval, direction="short", return_col=return_col, chunk_size=chunk_size, dtype=dtype
        )
    ...
    if verbose:
        print(f"[validacion_pl] LONG: {len(winners_long_list)} -> {len(best_long)} | SHORT: {len(winners_short_list)} -> {len(best_short)}")
    return {
        "best_long": best_long,
...
__all__ = ["run_pl_stability_selection", "select_stable_rules"]
