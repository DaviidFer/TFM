...
def _ensure_datetime_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(out.index, pd.DatetimeIndex):
        try:
            ...
            raise ValueError("El índice debe ser DatetimeIndex o parseable a fechas.") from e
    return out


def _ensure_target(df: pd.DataFrame, return_col: str) -> pd.DataFrame:
    """
    Asegura columna de returns tipo Target.
    Si return_col no existe, calcula desde open:
    ...
    if "open" not in out.columns:
        raise ValueError(
            f"No existe '{return_col}' ni columna 'open' para calcularla."
        )
...
def _profit_sum_and_coverage(rr_year: pd.DataFrame, eps: float = 1e-12):
    """
    rr_year: DataFrame de returns signados (long => +Target, short => -Target).
    """
...
def validate_forward_year_profitability(
    ...
    """
    Validacion forward del ultimo ano:
    - NO usa datos previos a 2025 (ni warmup, ni concatenaciones).
    - Evalua SOLO data_target_year (o solo las filas del year si se indica target_year).
    - Mantiene una regla si: sum(P/L) > 0 en ese ano.
    - Para short y long el signo ya lo gestiona build_rule_return_matrix(direction=...).
    """
    ...
    if not df_rules_long.empty:
        ...
        metrics_long = pd.DataFrame(
            {
                "regla": list(rr_long_year.columns),
                "profit_sum_year": profit_sum,
    ...
    if not df_rules_short.empty:
        ...
        metrics_short = pd.DataFrame(
            {
                "regla": list(rr_short_year.columns),
                "profit_sum_year": profit_sum,
                "coverage_year": coverage,
            }
        )
