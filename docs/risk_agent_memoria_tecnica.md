...
No se crea un motor paralelo de simulación. El `RiskAgent` reutiliza el backtest de eventos ya presente en `backtest_eventos/runner.py`, lo que evita duplicidades metodológicas y reduce el riesgo de inconsistencias entre el entorno de validación y el de supervisión forward.
...
`RiskLimitsConfig` agrupa la parametrización cuantitativa del agente. Entre sus parámetros más relevantes destacan:

- mínimo de trades forward para decisiones duras,
- umbrales de `health_score` para `DEGRADED`, `SUSPEND` y `RETIRE`,
- multiplicadores de drawdown respecto al diseño,
- ratios mínimos aceptables de profit factor y Sharpe respecto al baseline,
- máximo peso por trader,
- máximo peso por activo,
- exposición máxima total,
- buffer mínimo de caja,
- multiplicador de reducción para traders `DEGRADED`,
- umbral opcional de margen mínimo,
- umbral opcional de `emergency_drawdown_stop`.

El uso de un contrato explícito de límites simplifica la experimentación, la trazabilidad y la defensa académica del modelo.
...
Si el número de trades shadow es menor que `min_forward_trades_for_hard_decision`, se marca `insufficient_evidence`. En esta situación:

- se aplica penalización,
- se puede degradar el trader si el score cae lo suficiente,
- pero se evita llegar directamente a decisiones extremas como `RETIRE` sin soporte estadístico.

### 9.4. Transición entre estados

Con evidencia suficiente, el flujo general es:

1. `RETIRE` si hay suficiente muestra, score muy bajo y evidencia estructural de colapso.
2. `SUSPEND` si el score cae bajo el umbral intermedio.
3. `DEGRADED` si hay deterioro relevante pero no extremo.
4. `KEEP` si el trader se mantiene dentro de los márgenes aceptables.

Con evidencia insuficiente:

- se preserva un comportamiento más conservador,
- priorizando `KEEP` o `DEGRADED`.

### 9.5. Generación de `RetrainRequest`

Cuando la acción es `RETIRE`, la función crea un `RetrainRequest` con:
...
- número de traders en `LIVE`, `DEGRADED`, `SUSPENDED`, `RETIRED`,
