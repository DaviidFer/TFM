...
try {
    ...
    $dbPath = if ($env:TFM_DB_PATH) { $env:TFM_DB_PATH } else { Join-Path $projectDir "app\.tmp\supervisor\supervisor.sqlite" }
    $timestamp = Get-Date -Format "yyyyMMddTHHmmssZ"

    if (Test-Path $dbPath) {
        $sqliteUri = Get-S3Uri -Bucket $bucket -Prefix $prefix -Suffix "sqlite_backups/state_$timestamp.sqlite"
        aws s3 cp "$dbPath" "$sqliteUri"
    }
